...
- **Two surfaces in one process:**
  ...
  - REST at `/api/*` — handlers in [src/routes.ts](src/routes.ts) (login, enum lookups, read-only flags, `updateTemasAndVisitaAtendimento`).
...
- [routes.ts](src/routes.ts) — REST endpoints. Each handler instantiates Prisma + repos at module scope and delegates to repositories or `LoginService`.
...
- [repositories/](src/repositories/) — `Repository.ts` is a loose structural interface; [repositories/prisma/](src/repositories/prisma/) holds the implementations. Every repo extends [PrismaRepository](src/repositories/prisma/PrismaRepository.ts) (shared Prisma client + error translation).
...
- All Prisma access goes through a `repositories/prisma/*Repository.ts`. Do not `new PrismaClient()` from inside a resolver or module file — reuse [src/config/prismaClient.ts](src/config/prismaClient.ts) or extend `PrismaRepository`.
...
- **No comments by default.** Add a one-liner only when *why* is non-obvious (hidden constraint, subtle invariant, workaround for a specific bug). Never narrate *what* the code does.
